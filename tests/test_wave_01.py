...
def test_moves_movie_from_watchlist_to_empty_watched():
    ...
    assert len(updated_data["watched"]) == 1
    # ****** Add assertions here to test that the correct movie was added to "watched" ********** AT
    assert updated_data["watched"][0]["title"] == MOVIE_TITLE_1
    assert updated_data["watched"][0]["genre"] == GENRE_1
    assert updated_data["watched"][0]["rating"] == RATING_1
...
def test_moves_movie_from_watchlist_to_watched():
    ...
    # Assert
    assert len(updated_data["watchlist"]) == 1
    assert len(updated_data["watched"]) == 2 #this indicates two movies watched
    # ****** Add assertions here to test that the correct movie was added to "watched" ********** AT
    assert updated_data["watched"][0] == FANTASY_2 #we need to assert both expected movies 
    assert updated_data["watched"][1]== HORROR_1 #the function returns two movies watched 
